...
## [1.6.11.13] · 2026-09-01 · 导航、交互与评论兼容稳定版

### 累计内容

- 汇总 rc.01～rc.03 的配置契约、PluginCommentWidget 3.2.2 兼容、导航与键盘无障碍、分页及加载更多/点赞/分享交互修复。
- 纳入 LCP 候选、媒体懒加载、图库与瞬间开销收敛，以及包内资源版本缓存键更新。
- 保留移动抽屉跨重复初始化的焦点代次控制，并以独立行为夹具和 mutation 守住取消、后项胜出及隐藏候选过滤。

### 验证边界

- stable 与已发布 rc.03 使用相同主题实现，仅收敛版本号和缓存键；已通过本地门禁及隔离 Halo 2.25.4/2.26.0 覆盖升级、桌面和 390px 验证，未宣称生产部署。

---

## [1.6.11.13-rc.03] · 2026-09-01 · 重复初始化焦点候选版

### 修复

- 让移动菜单的焦点代次在重复初始化之间保持共享；新的初始化、关闭或再次打开都会取消旧实例尚未执行的延迟焦点任务，避免焦点落入过期抽屉。

### 开发与验证

- 将抽屉焦点控制器的行为验证拆为 `beginInit` 取消、`invalidate` 取消、连续调度后项胜出及跨初始化共享四类独立夹具；每类 generation mutation 都有对应失败诊断，同时保留隐藏候选过滤的逐项 mutation。

---

## [1.6.11.13-rc.02] · 2026-09-01 · 抽屉焦点强化候选版

### 修复

- 为移动菜单焦点调度加入代次校验，快速“打开 → 关闭 → 打开”时只允许当前任务聚焦一次。
- 聚焦前重新检查抽屉状态，并排除无布局框、`hidden`、`inert`、`aria-hidden` 或 `visibility: hidden` 链路中的候选元素。

### 开发与验证

- 增加双 `requestAnimationFrame` 时序夹具及 generation、布局框和各类隐藏祖先的独立 mutation，防止取消逻辑或候选过滤被无意移除。

---

## [1.6.11.13-rc.01] · 2026-08-31 · 键盘焦点候选版

### 修复与兼容性

- 明确主题配置契约和缺省行为：全局侧栏、页脚和广告等总开关现在按实际配置统一约束下级内容；旧百度 Token 仅保留配置兼容，不再输出或参与运行时逻辑。
- `enable_copy` 的文章复制缺省值由关闭调整为开启，仅影响新安装或尚未保存该项的缺省配置；旧用户已经显式保存的开关值不会被覆盖。
- 完善 Halo 默认评论的“评论后展开”兼容路径，支持 PluginCommentWidget 3.2.2 的延迟挂载与真实 Shadow DOM 根节点；组件、挂载点或请求状态异常时保持正文可见。
- 补齐共享 `main`、skip link、移动菜单和移动目录的键盘导航、ARIA 状态、互斥打开、滚动恢复与 Escape 回焦；skip link 会同步片段地址并将焦点移到主要内容。
- 将加载更多、文章和瞬间点赞、评论展开、微信分享及图库筛选改为可感知状态的原生按钮；加载更多补充忙碌状态、结果播报和重复请求保护，分享菜单支持键盘焦点展开。
- 统一首页、作者页、归档、分类、标签、瞬间等分页的导航标签、当前页、禁用项和省略号语义，避免禁用链接继续可聚焦或跳转。

### 性能

- 在无大横幅时提升首个有效轮播图或文章封面为 LCP 候选，使用 eager loading 和高请求优先级，并同步真实 `src` / `srcset`。
- 为 PDF iframe 和非首屏媒体补充懒加载；收紧图库与瞬间的动画和事件初始化，减少不可见内容、重复处理及无消费者资源带来的开销。

### 开发与验证

- 修正本地 `theme-sync.sh` 健康检查固定命中宿主 8090 的问题：检查现在绑定目标容器的有效 published port，规范处理 IPv4/IPv6 映射、端口范围、代理绕过和有界超时诊断。
- 扩充 source/package verifier 的可执行契约和 mutation 覆盖，包含配置缺省值、评论 3.2.2、导航与交互状态、分页、LCP、媒体懒加载、脚本时序以及源码与 ZIP 一致性。
- 为包内默认图片和按需脚本统一主题版本缓存键，同时保持用户自定义资源 URL 原样，避免升级后继续复用旧资源或改写外部地址。

---

## [1.6.11.12] · 2026-08-21 · 稳定版

### 修复与无障碍

- 恢复 jQuery 头部同步加载，确保正文渲染插件插入的同步脚本可以可靠使用 `$`；其余主题脚本继续按依赖顺序延迟执行，并修正文章目录 `enable_toc` 与正文、侧栏分享 `enable_share` 的缺省行为。
- 将操作菜单、移动端菜单、归档折叠和相册筛选改为原生按钮，补齐 ARIA、`inert`、焦点返回、`focus-visible` 与触控目标，并让动画、滚动、轮播和目录交互尊重系统“减少动态效果”偏好。
- 完善 390px 移动端布局，修正独立 404 错误页的浏览器默认 `body` 外边距，消除 8px 横向滚动。

### 性能

- 收窄列表封面、WOW.js 与 Animate.css 的下载条件，并压缩两张默认占位 GIF；保留文件名、帧数、时长和循环行为。
- 移除相册页无活动消费者的图库依赖，统一音乐播放器资源加载，并让 Beauty、微信二维码、留言板脚本和普通页面依照真实 DOM 与功能开关按需加载。
- 为默认占位资源、核心脚本、鼠标指针和背景特效补充版本化缓存键；用户填写的自定义资源 URL 保持原样。

### 维护

- 主题卡片 Logo 改用包内资源；已保存的旧 Joe3 上游仓库、GitHub Pages 或 jsDelivr 资源路径自动回退到 `/themes/theme-Joe3`，降低上游删库对现有站点的影响。
- Font Awesome 仅保留完整图标映射及 WOFF2 字体；打包改用锁定版本的纯 Node `fflate`，排除开发 YAML 和旧字体格式，并通过原子替换生成主题 ZIP。
- 扩充 source/package verifier，覆盖插件与脚本时序、配置缺省值、按需资源、缓存键、无障碍语义、404 模板完整 SHA-256、主题身份及源码与 ZIP 字节一致性。

### 验证与兼容性

- `metadata.name`、`settingName`、`configMapName` 与本地主题路径保持不变，Halo 原地覆盖升级和既有后台配置继续兼容；rc.07 已在 Halo 2.25.4 从旧版本真实覆盖升级，主题激活状态与配置均保留。
- rc.08 公开包已确认与本地运行内容逐字节一致，390px 页面实测通过；本条不代表 rc.08 另行完成过 Halo Console 覆盖升级。
- 自建 CDN 地址仍受支持；沿用旧上游路径片段的自建镜像会回退到包内资源。普通 `sheet` 不再隐式提供 marked，相关自定义脚本需自行加载。
- IE 11、Safari 9 及更早版本和部分旧 Android 浏览器不支持 WOFF2，Font Awesome 图标可能无法显示；现代浏览器不受影响。

---

## [1.6.11.12-rc.08] · 2026-08-21 · 移动端 404 页面候选修复

### 修复

- 修正独立 404 错误页未重置浏览器默认 `body` 外边距，导致 390px 移动端产生 8px 横向滚动的问题。
...
## [1.6.11.12-rc.05] · 2026-08-18 · 文章配置与留言板资源候选版

### 修复

- 修正文章目录配置错误读取文章评论标注的问题；文章级目录开关现在读取 `enable_toc`，缺少标注时继续按启用处理。
- 统一文章正文与侧栏分享入口的缺省行为；文章没有 `enable_share` 标注时，两处现在都与标注默认值一致，按启用分享处理。

### 性能

- 普通 `sheet` 页面不再加载没有活动消费者的 marked，也不再加载仅留言板需要的 Draggabilly 和 leaving 脚本。
- `page_leaving` 的默认评论分支在留言 DOM 已存在后，按 Draggabilly、版本化 leaving 的顺序各加载一次。
- Waline 留言板先写入返回的留言数据，再按顺序各加载一次 Draggabilly 和版本化 leaving；请求或脚本加载失败不会中断页面其余功能。

### 维护

- 主题包校验新增文章目录与分享缺省值、留言板默认/Waline 分支矩阵、资源全局加载集合与顺序、leaving 源码/构建产物活动消费者，以及源码与 ZIP 一致性守卫。

### 兼容性提示与已知限制

- 主题不再为普通页面全局提供 marked。若非公开的自定义 JavaScript 曾隐式依赖该全局对象，需要自行按需加载 marked。
- 本候选版只修正 Post 的目录标注读取，不宣称处理 SinglePage 的目录逻辑；本次也是留言板资源范围的定向调整，不代表完成全站资源重构。

---

## [1.6.11.12-rc.04] · 2026-08-17 · 资源条件与缓存守卫候选版

### 修复

- 微信二维码库现在仅在文章页同时满足全局分享、微信分享和文章级分享标注四层条件时加载，并与正文及侧栏的两个真实二维码入口保持一致。
- Beauty 样式、脚本和初始化按实际页面生产者收窄：友链页保留卡片所需样式，大横幅脚本仅在启用大横幅且页面确实输出横幅时运行；作者页继续受大图标签开关约束。

### 性能

- 相册页移除已不再使用的 Justified Gallery 样式与脚本、Masonry 和 imagesLoaded 共 4 项依赖；继续保留打包内的 Isotope，并保持其在相册页脚本之前按需加载。
- 全局音乐播放器的 APlayer 样式、APlayer 脚本和 Meting 脚本统一为一组条件加载，避免同一页面重复引入。
- NProgress 在启用时继续位于头部依赖链，但改用 `defer`，并保持在页尾 `common.min.js` 执行前可用。
- 鼠标指针和背景特效改为带主题版本号的缓存脚本请求，同一版本可复用浏览器缓存，版本升级后自动换键。
- 内置默认占位 GIF 的资源地址增加主题版本参数；用户填写的自定义资源 URL 保持原样，不追加或改写查询参数。
- 本版为 `utils.min.js` 和按条件加载的 `beauty.min.js` 新增主题版本参数；`common.min.js` 继续保留已有版本参数，减少主题升级后继续命中旧脚本的风险。

### 维护

- 主题包校验新增相册依赖边界、默认与自定义资源 URL、音乐资源去重、NProgress 时序、特效缓存、Beauty 页面条件、二维码 DOM 与运行时控制流，以及核心脚本全模板唯一加载守卫。
- source 校验兼容尚未生成 gitignored min 文件的干净检出；package 校验仍要求构建产物齐全。ZIP 字节一致性范围包括 `settings.yaml`、`common.js`、`post.js`、本版新增的 `utils.js` 与 `beauty.js`、`custom.min.js`，以及 `common.min.js`、`post.min.js`、`utils.min.js`、`beauty.min.js`，同时继续覆盖资源加载模板和默认占位资源生产模板。

### 已知限制

- 本候选版是资源加载范围、缓存键和打包回归的定向调整，不代表完成全站性能重构或完整无障碍审计；既有第三方依赖和历史交互仍需后续逐项处理。

---

## [1.6.11.12-rc.03] · 2026-08-14 · 键盘交互与界面细节候选版

### 修复

- 将操作菜单、移动端菜单、归档时间线折叠/展开和相册筛选的触发器改为原生按钮，并补齐 ARIA 状态、非活动区域 `inert` 管理及关闭后的焦点返回。
- 将分页省略号改为非交互文本，避免键盘用户聚焦到不可操作项目；键盘快捷处理同时放行原生控件的空格键行为。
- 彩带特效脚本在缺少博主侧栏 Canvas 的页面跳过初始化，并为资源增加版本参数，避免相册页报错或继续执行旧缓存。

### 视觉

- 微调浅色与深色模式的次要文字对比度，并为页头和内容表面增加轻量层级；评论标题移除位图装饰。
- 普通动态偏好下原有动画节奏与主题主色保持不变。

### 性能

- 将 `lazyload.gif` 与 `lazyload_h.gif` 的合计体积从 240,325 B 压缩至 82,581 B（减少 65.64%），保留原文件名、116 帧、3.54 秒时长和循环播放行为。

### 无障碍

- 为键盘焦点增加兼容浅色与深色背景的双色 `focus-visible` 焦点环，并改善相关控件的触控目标尺寸。
- 尊重系统“减少动态效果”偏好，让主题过渡即时完成；本候选版聚焦当前交互路径，不代表已禁用所有特效或解决全部无障碍问题。

### 维护

- 主题包校验新增键盘交互、ARIA、视觉对比度与表面层级、GIF 帧结构与 LZW 数据，以及两张默认 GIF 源文件与 ZIP 内容一致性守卫。

---

## [1.6.11.12-rc.02] · 2026-08-13 · 列表性能与归档语义候选版

### 修复

- 文章列表封面将响应式图片候选集改为真正的延迟加载，避免浏览器被 `srcset` 提前触发下载。
- 修正分类页的默认标题文案，并改用作者信息生成作者归档页标题。

### 性能

- WOW.js 仅在动态页、朋友圈或首页的对应动画开关启用时加载，避免动画关闭或访问其他页面时下载无用脚本。
- Animate.css 仅在页面存在第三方动画类依赖时加载，默认关闭动画的列表页不再下载和解析无用样式。

### 无障碍

- 尊重系统“减少动态效果”偏好，将主题 CSS 动画的持续时间与延迟压缩为单次瞬时完成，并让主题 JS 平滑滚动回退为即时滚动，避免内容因直接禁用动画而保持隐藏。

### 维护

- 统一三项动画开关的布尔解析，并为列表封面延迟加载、动画资源条件、减少动态效果与归档标题新增主题包校验守卫。

---

## [1.6.11.12-rc.01] · 2026-08-13 · 模板语义与插件兼容候选版

### 修复

- 将页面配置脚本改为唯一 ID，并将文章页两处微信分享入口改用共享 class，避免原始 HTML 出现重复 ID。
- 将 jQuery 恢复为头部同步加载，确保正文渲染插件插入的同步脚本执行时可以使用 `$`；其余主题脚本继续在页尾延迟执行。

### 性能

- 为主题固定尺寸的博主头像、头像挂架、等级图标、加载动画和 404 插图补充尺寸属性，减少图片加载前后的布局跳动。

### 维护

- 主题包校验新增配置脚本 ID、微信分享选择器和 jQuery 唯一加载位置守卫，防止相同问题回归。
- 精确升级构建链中的 PostCSS、js-yaml、Less、brace-expansion 与 nanoid，移除已知高危依赖；Less 同步改用不依赖 image-size 的版本。

---

## [1.6.11.11] · 2026-07-23 · 精品分类图片回归修复

### 修复

- 让精品分类懒加载同时替换 `src` 与 `srcset`，避免 Halo 图片处理生成的占位图 `srcset` 覆盖真实分类封面。

### 维护

- 主题包校验新增精品分类 `data-src` / `data-srcset` 成对存在守卫，防止相同回归再次发布。

---

## [1.6.11.10] · 2026-07-23 · 首屏性能与无障碍优化

### 修复

- 允许移动端页面缩放，并把字符集声明提前到文档头部，改善可访问性与浏览器解析。
- 目录背景图使用裁切适配，避免固定尺寸造成图片比例失真。
- 修正自定义热门分类图片引用未定义 `category` 变量的问题，并统一改为真正的延迟加载。
- 为分页前后按钮补充可访问名称，将 NProgress 的无效 `role="bar"` 改为标准进度条角色。
- 扩大广告关闭按钮点击区域，修复文章列表元信息和页脚的文字对比度。

### 性能

- 将 jQuery 从文档头部移动到页面末尾，所有外部主题脚本统一使用 `defer` 并保持依赖顺序；消息提示与图片灯箱样式改为非阻塞加载，减少首屏渲染等待。
- 首页仅优先加载首张轮播图并设置 `fetchpriority="high"`，其余轮播图改用 `data-srcset` 延迟下载，减少首屏图片竞争。

### 维护

- 主题包校验新增 jQuery 加载位置、全量 `defer` 和依赖顺序守卫。
- 通过精确 overrides 将 `brace-expansion` 固定到 2.1.2、`svgo` 固定到 2.8.3，解除依赖审计阻塞，不引入 major 升级。

---

## [1.6.11.10-rc.01] · 2026-07-13 · 移动端可访问性与首屏性能候选版
...
- 主题包校验新增 jQuery 加载位置、全量 `defer` 和依赖顺序守卫。
- 同步当前 Release、Dependabot PR 与交接状态。

---

## [1.6.11.9] · 2026-07-12 · Halo 2.25.4 兼容与资源瘦身

### 修复

- PDF 嵌入初始化不再写入全局 `options`，并对带查询参数的 PDF 地址正确编码。
- 首页自动加载仅在首页启用，补齐空节点、重复请求、HTTP 错误和失败重试处理。
- 修正独立页面阅读限制区域多输出一个 `>` 的模板错误。
- 文章或独立页面关闭字数统计时，不再因目标节点缺失产生前端异常。

### 安全

- 兰空图床 Token 设置增加浏览器暴露警告，并移除真实 Token 格式示例。

### 维护

- 删除未参与构建的 `rollup-plugin-less` 和 npm 侧 Waline 客户端依赖；前台 Waline 默认地址继续锁定 v2。
- 更新 Halo 打包 CLI、Less、Prettier 与存在安全公告的传递依赖，`pnpm audit` 恢复为零告警。
- CI 新增格式、依赖审计、主题身份、禁用 app-id、CHANGELOG、annotated tag、版本和最终 zip 校验。
- Dependabot 按样式、打包、bundler 分组，忽略当前 Node 20 不兼容的 lint-staged major，并停止引用不存在的标签。
- 将阿里云盘图标从内联 Base64 提取为静态资源，显著降低全局 CSS 传输体积。
- 删除未使用的 Vue 2、PDF.js 演示文件、旧说明截图及第三方库开发副本，主题包由 15.42 MB 降至 13.53 MB。
- 主题包校验新增 JavaScript 语法、超大内联图片和冗余开发资产检查。
- 刷新 Browserslist 数据，并修正 Issue 模板占位符和无效联系链接。

---

## [1.6.11.8] · 2026-06-01 · 发布直链与 Waline 稳定性

### 修复

- Waline 留言页改为转义评论正文、昵称和头像地址，只允许安全 URL 协议，避免不受信内容直接拼入 HTML。
- Waline 留言 API 的页面路径改为 URL 编码，并兼容接口缺少评论数组的情况。
- Waline 功能脚本默认地址明确锁定 `@waline/client@v2`，避免 CDN 自动漂移到 major 新版。
- `beauty.min.css` 仅在启用大横幅或友链页时加载，减少无关页面资源请求。

### 发布

- Release 正文新增可直接复制的主题 zip 下载链接，并同步写入 workflow summary。

---

## [1.6.11.7] · 2026-05-15 · 可见性兜底与资源瘦身

### 安全

- 文章列表、归档、轮播、热门文章、相关推荐、上下篇和动态模板统一增加 `PUBLIC` 白名单过滤。
- 最新评论侧栏在渲染前检查关联文章或页面可见性，并对 Waline 评论内容和 URL 做安全处理。

### 优化

- 删除主题未引用的本地 Halo Comment、KaTeX 等资源，安装包由约 22 MB 降至 16.4 MB。
- 修正动态作者链接、安全补充外链 `rel`，并将默认随机图接口切换为 HTTPS。
