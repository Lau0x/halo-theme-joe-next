...
- 工作区干净；`main` 目前领先 origin 若干未推的 docs commit（交接文档），**未 push**。
...
- **#10** `rolldown` 1.0.0-beta.58 → 1.0.1：CI 红，lockfile overrides 与 `package.json` 不匹配。
- **#3** `@waline/client` 2.15.8 → 3.13.0：CI 绿，但 v3 major，需专项测试，不当顺手依赖升。

## 开工检查（每次动手前）

```bash
cd /Users/royleo/Documents/CC/codextocc/halo_theme/halo-theme-joe-next
git status -sb
git log --oneline -8
